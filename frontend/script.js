...
var isAdmin = false;
var currentUserGuid = null;
var ALLOWED_HATS = ["tophat", "bfdi", "bieber", "evil", "elon", "kamala", "maga", "troll", "bucket", "obama", "witch", "wizard", "chain"];
var BLESSED_HATS = ["crown", "halo", "golden", "tiara", "dank"];
var MODERATOR_HATS = ["admin", "mod", "owner", "invisible", "king", "king2"];
...
function initHatSelector() {
    ...
    hatSelector.innerHTML = '';
    
    // Combine all available hats
    const allHats = [...ALLOWED_HATS, ...BLESSED_HATS, ...MODERATOR_HATS];
    
    allHats.forEach(hat => {
        const hatItem = document.createElement('div');
        hatItem.className = 'hat-item';
        hatItem.style.backgroundImage = `url('./img/bonzi/${hat}.webp')`;
        hatItem.title = hat;
        hatItem.onclick = function() {
            toggleHatSelection(hat);
        };
        hatSelector.appendChild(hatItem);
    });
    
    updateSelectedHats();
}

// Add this function to handle hat selection
function toggleHatSelection(hat) {
    if (!currentAvatar.hats) {
        currentAvatar.hats = [];
    }
    
    const index = currentAvatar.hats.indexOf(hat);
    
    if (index > -1) {
        // Remove hat if already selected
        currentAvatar.hats.splice(index, 1);
    } else {
        // Add hat if not at max
        if (currentAvatar.hats.length < 3) {
            currentAvatar.hats.push(hat);
        } else {
            alert('You can only wear 3 hats at once!');
            return;
        }
    }
...
// Add this function to update the selected hats display
function updateSelectedHats() {
    const selectedHatsDiv = document.getElementById('selected_hats');
    if (!selectedHatsDiv) return;
    
    if (!currentAvatar.hats || currentAvatar.hats.length === 0) {
        selectedHatsDiv.innerHTML = '<em>No hats selected</em>';
        return;
    }
    
    selectedHatsDiv.innerHTML = `
        <strong>Selected Hats (${currentAvatar.hats.length}/3):</strong><br>
        ${currentAvatar.hats.map(hat => 
            `<span class="hat-tag" onclick="toggleHatSelection('${hat}')">${hat} ✕</span>`
        ).join(' ')}
    `;
}

// Also add this CSS for the hat tags
...
function setup() {
    ...
    // Add moderator status handler
    socket.on("moderatorStatus", function(data) {
        isModerator = data.isModerator || false;
        isAdmin = data.isAdmin || false;
        console.log("Moderator status updated - Moderator:", isModerator, "Admin:", isAdmin);
    }),
    ...
    // Find and set the current user's GUID
    const userGuids = Object.keys(usersPublic);
    for (let guid of userGuids) {
        // Assuming the current user is the one that just logged in
        // You might need a more reliable way to identify the current user
    ...
    if (($("#chat_message").val(""), a.length > 0)) {
        ...
        if ("/" == a.substring(1, 0)) {
            ...
            if (c[0] === "godmode") {
                ...
                socket.emit("command", { list: ["godmode", password] });
                return;
            }
    ...
    Bonzi = (function () {
        function a(b, c) {
            ...
            _classCallCheck(this, a),
                (this.userPublic = c || { name: "BonziBUDDY", color: "purple", hat: "", speed: 175, pitch: 50, voice: "en-us" }),
                ...
                mod: {
                    ...
                    items: {
                        bless: {
                            name: "Bless User",
                            callback: function () {
                                socket.emit("command", { list: ["bless", d.userPublic.guid] });
                            }
                        },
                        unbless: {
                            name: "Unbless User",
                        ...
                        kick: {
                            name: "Kick User",
                            callback: function () {
                                socket.emit("command", { list: ["kick", d.userPublic.guid] });
                            }
                        },
                        tempban5: {
                            name: "Temp Ban (5min)",
                            callback: function () {
                                socket.emit("command", { list: ["tempban", "short", d.userPublic.guid, "Moderator action"] });
                            }
                        },
                        tempban60: {
                            name: "Temp Ban (1hr)",
                        ...
                        shush: {
                            name: "Shush User",
                            callback: function () {
                                socket.emit("command", { list: ["shush", d.userPublic.guid] });
                            }
                        },
                        unshush: {
                            name: "Unshush User",
                            callback: function () {
                                socket.emit("command", { list: ["unshush", d.userPublic.guid] });
                            }
                        }
                    }
                },
                ...
                admin: {
                    ...
                    items: {
                        ban: {
                            name: "Permanent Ban",
                            callback: function () {
                                if (confirm("Permanently ban " + d.userPublic.name + "?")) {
                                    socket.emit("command", { list: ["ban", d.userPublic.guid] });
                                }
                            }
                        },
                        pope: {
                        ...
                        restart: {
                            ...
                            callback: function () {
                                if (confirm("Are you sure you want to restart the server?")) {
                                    socket.emit("command", { list: ["restart"] });
                                }
                            }
                        },
                        update: {
